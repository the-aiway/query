...
var require_node = __commonJS({
  "../../node_modules/web-worker/cjs/node.js"(exports2, module2) {
    ...
    var VM = require("vm");
    var threads2 = require("worker_threads");
    ...
    module2.exports = threads2.isMainThread ? mainThread() : workerThread();
    ...
    function mainThread() {
      class Worker2 extends EventTarget {
        constructor(url, options) {
          ...
          const worker = new threads2.Worker(__filename, {
            workerData: {
              mod,
              name,
              type
    ...
    function workerThread() {
      ...
      threads2.parentPort.on("message", (data) => {
        const event = new Event("message");
        event.data = data;
        if (q == null)
          self.dispatchEvent(event);
      ...
      threads2.parentPort.on("error", (err) => {
        err.type = "Error";
        self.dispatchEvent(err);
      });
      class WorkerGlobalScope extends EventTarget {
        postMessage(data, transferList) {
          threads2.parentPort.postMessage(data, transferList);
        }
...
var AsyncPreparedStatement = class {
  ...
  async query(...params) {
    ...
    this.bindings.logger.log({
      ...
      topic: 4 /* QUERY */,
      event: 4 /* RUN */,
      value: `Prepared statement ${this.statementId}`,
      id: queryId
    });
    try {
...
var AsyncDuckDB = class {
  ...
  shouldOPFSFileHandling() {
    ...
    if (isOPFSProtocol(this.config.path ?? "")) {
      return ((_a = this.config.opfs) == null ? void 0 : _a.fileHandling) == "auto";
    }
    return false;
  }
