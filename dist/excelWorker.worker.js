...
// client/lib/services/excelWorker.worker.ts
var API_BASE_URL = "";
var token = "";
...
function maybeUpdateAuthAndBase(data) {
...
self.addEventListener("message", async (e) => {
  try {
    const data = e.data;
    const { type } = data;
    maybeUpdateAuthAndBase(data);
    if (type === "parse") {
      postMessage({ type: "downloading" });
      const fetchUrl = resolveFetchUrl(data.url);
      const headers = {};
      if (token && API_BASE_URL && fetchUrl.startsWith(API_BASE_URL)) {
        headers.Authorization = `Bearer ${token}`;
      }
      const response = await fetch(fetchUrl, Object.keys(headers).length ? { headers } : undefined);
      const arrayBuffer = await response.arrayBuffer();
      postMessage({ type: "parsing" });
      const wb = readSync(arrayBuffer, { type: "array", dense: true, raw: true });
      ...
      postMessage({ type: "parsed", sheets });
    } else if (type === "parseBuffer") {
      postMessage({ type: "parsing" });
      const wb = readSync(data.buffer, { type: "array", dense: true, raw: true });
      const sheets = wb.SheetNames.map((sheetName) => {
        const ws = wb.Sheets[sheetName];
        if (!ws)
          return { name: sheetName, data: [] };
