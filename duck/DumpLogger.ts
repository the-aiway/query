...
export class DumpLogger implements QueryLogger {
  ...
  async logQuery(query: string, queryFn: () => Promise<ArrowTable<any>>, db?: AsyncDuckDB): Promise<ArrowTable<any>> {
    ...
    const hangingTimer = setTimeout(() => {
      console.log(`%c${_id}%c ⏳ Hanging: ${queryStart}`, 'color: #888; font-weight: bold', 'color: #f59e0b; font-style: italic');
    }, 1492);
    ...
    try {
      ...
      console.groupCollapsed(`%c${_id}%c ✓ ${queryStart} %c(${duration}ms)`, 'color: #888; font-weight: bold', 'color: #10b981', 'color: #666; font-style: italic');
      ...
      clearTimeout(hangingTimer);
      const duration = (performance.now() - start).toFixed(1);
      const errMessages = Array.from(new Set((error as Error).message.split('\n').filter((e: string) => e.trim())));
      console.groupCollapsed(`%c${_id}%c ❌ Error: ${queryStart} %c(${duration}ms)`, 'color: #888; font-weight: bold', 'color: #ef4444', 'color: #666; font-style: italic');
      console.log(highlightedQuery);
