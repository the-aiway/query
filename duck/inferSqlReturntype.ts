...
type FillUnknown<T, Fill> = {
  [K in keyof T]: unknown extends T[K] ? Fill : T[K];
};
...
type ResolveCast<T extends string> =
  Trim<T> extends `${infer Base}[]`
    ? ResolveCast<Base>[]
    : Lowercase<StripParens<Trim<T>>> extends keyof CastMap
      ? CastMap[Lowercase<StripParens<Trim<T>>>]
      : unknown;
...
export type InferSQL<T extends string, TOverride = unknown> = unknown extends TOverride
  ...
  : TOverride extends unknown[]
    ...
    : TOverride extends Record<string, unknown>
      ...
      : Materialize<FillUnknown<InferSql<T>[number], TOverride>>[];
...
export type InferDuckType<T extends string> = ResolveCast<T>;

export type InferDuckTable<T extends Record<string, string>> = Materialize<{
  [K in keyof T]: InferDuckType<T[K]>;
}>;
...
`);
  ...
  // test48 satisfies { '1': string }[];

  // const test49 = sql<string>('SELECT A, B::int FROM TOTO');
  // test49 satisfies { A: string; B: number }[];
  ...
  // sql<bigint>() loses literal type (TS generic inference limitation)
  // Use .rows<bigint>() on the ref instead — tested via FillUnknown:
  type _test53_row = InferSQLStrict<'SELECT whatever FROM TOTO'>[number];
  type _test53_filled = Materialize<FillUnknown<_test53_row, bigint>>;
  null! as _test53_filled satisfies { whatever: bigint };
  ...
  //   zone_count: 'float',
  //   carrier_name: 'text',
  //   available_carriers: 'text[]',
  // });
  // runsLiteStore satisfies Ref<{
  //   cost: number;
  //   assignment: string;
  //   total_cost: number;
  //   zone_count: number;
