...
function useColumnOptions(opts: {
  ...
  const counts = useSql(
    (
      t,
    ) => `SELECT COALESCE(CAST(${t.raw.colIdent} AS VARCHAR), '__NULL__') AS key, COUNT(*)::BIGINT AS cnt
...
export function OptionsFilter({
  ...
  const selectedKeys = useMemo(
    () => (filterValue && isSetFilter(filterValue) ? new Set(filterValue) : new Set<string>()),
    [filterValue],
  );
  ...
  const [options] = useSql((t) => (open ? `SELECT * FROM ${t.options}` : "SELECT unnest([]) "), {
    options: optionsRef,
    open,
  }).useRows((arr) =>
    arr?.map(
    ...
    <Popover open={open} onOpenChange={(o) => onOpenFilterCol(o ? col : null)}>
      <PopoverTrigger asChild>
        <button
          ...
          title={
            filterValue && isSetFilter(filterValue)
        ...
        >
          ...
          {filterValue && isSetFilter(filterValue) && filterValue.length > 0 && (
            <span className="bg-primary text-primary-foreground absolute -top-1 -right-1 h-4 min-w-4 rounded-full px-1 text-center text-[10px] leading-4">
              {filterValue.length}
            </span>
          )}
        </button>
      ...
      <PopoverContent
        align="start"
        side="bottom"
        className="bg-background/95 supports-backdrop-filter:bg-background/80 w-[420px] p-3 backdrop-blur"
                ...
                return (
                  ...
                  >
                    <div className="pt-0.5">
                      <div
                        className={`h-4 w-1.5 min-w-1.5 rounded-full ${checked ? "bg-primary" : "bg-muted-foreground/30"}`}
                      />
                    </div>
